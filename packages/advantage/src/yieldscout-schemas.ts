...
const periodBasis = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("per_second"), periodsPerYear: z.literal("31536000"), source }).strict(),
  z.object({ kind: z.literal("per_block"), periodsPerYear: positiveBaseUnits, source }).strict(),
])
...
const market = z
  .object({
    ...
    supplyState: z.enum(["active", "paused", "unknown"]),
    withdrawalState: z.enum(["active", "paused", "unknown"]),
...
const costValuation = z.discriminatedUnion("status", [
  z.object({ status: z.literal("current"), asset, observedAtUtc: utc, source }).strict(),
  z.object({ status: z.literal("unknown"), asset, observedAtUtc: z.null(), source: z.null() }).strict(),
])

export const yieldScoutEvaluationInput = z
  .object({
...
export const yieldScoutEvaluationArtifact = z
  .object({
    schemaVersion: z.literal("knot.yield.artifact/1"),
    category: z.literal("yield"),
    capability: z.literal("analysis"),
    taskId: z.string().nullable(),
    status: z.enum(["ASSESSED", "NO_ACTION", "NO_ALTERNATIVE", "STALE_SNAPSHOT", "ASSESSMENT_INCOMPLETE", "UNSUPPORTED_POSITION", "INVALID_REQUEST"]),
    reasonCode: z.enum(["ANALYSIS_ONLY", "AUTHORITY_MISMATCH", "BELOW_MINIMUM_IMPROVEMENT", "COSTS_DOMINATE", "COST_VALUATION_UNKNOWN", "CURRENT_MARKET_DATA_INCOMPLETE", "CURRENT_MARKET_NOT_FOUND", "CURRENT_MARKET_UNSUPPORTED", "INVALID_JSON", "NO_ELIGIBLE_ALTERNATIVE", "SCHEMA_VALIDATION_FAILED", "SNAPSHOT_TARGET_MISMATCH", "STALE_COST_VALUATION", "STALE_SNAPSHOT"]).nullable(),
    assessedAtUtc: utc,
    evidence: z
      .object({ snapshotId: z.string().min(1), chainId: z.literal(56), blockNumber: positiveBaseUnits, blockHash: digest, blockTimestampUtc: utc, capturedAtUtc: utc, sources: z.array(z.string().min(1)).min(1) })
      .strict()
      .nullable(),
    currentMarketId: z.string().nullable(),
    selectedMarketId: z.string().nullable(),
    eligibleMarkets: z.array(yieldComparison),
    excludedMarkets: z.array(z.object({ marketId: z.string(), reasons: z.array(z.string().min(1)).min(1) }).strict()),
...
export type YieldScoutEvaluationInput = z.infer<typeof yieldScoutEvaluationInput>
export type YieldScoutEvaluationArtifact = z.infer<typeof yieldScoutEvaluationArtifact>
export type YieldEvaluationMarket = YieldScoutEvaluationInput["snapshot"]["markets"][number]
export type YieldComparison = z.infer<typeof yieldComparison>
