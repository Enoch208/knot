...
import { alignedCeil, alignedFloor, amountsForLiquidity, MAX_TICK, MIN_TICK, sqrtRatioAtTick } from "./tick-math.ts"
...
export function analyzeRangePilotText(text: string, now = new Date()): string {
  ...
  if (!parsed.success) return JSON.stringify(refusal("INVALID_REQUEST", "SCHEMA_VALIDATION_FAILED", now, null))
...
export function analyzeRangePilot(request: RangePilotRequest, now = new Date()): RangePilotArtifact {
  ...
  if (identityFailure !== null) return refusal("UNSUPPORTED_POSITION", identityFailure, now, evidence, task.taskId)
  const stateFailure = stateFailureCode(request)
  if (stateFailure !== null) return refusal("UNSUPPORTED_POSITION", stateFailure, now, evidence, task.taskId)
  if (!isFresh(request, now)) return refusal("STALE_SNAPSHOT", "STALE_SNAPSHOT", now, evidence, task.taskId)
  ...
  const condition =
    snapshot.pool.currentTick >= snapshot.position.tickLower && snapshot.pool.currentTick < snapshot.position.tickUpper
      ? "IN_RANGE"
      : "OUT_OF_RANGE"
  const currentState = {
  ...
  const proposedAmounts = amountsForLiquidity(liquidity, BigInt(snapshot.pool.sqrtPriceX96), bounds.lower, bounds.upper)
  const available0 =
    BigInt(snapshot.pool.token0.balanceUnits) + currentAmounts.amount0 + BigInt(snapshot.position.tokensOwed0)
  const available1 =
    BigInt(snapshot.pool.token1.balanceUnits) + currentAmounts.amount1 + BigInt(snapshot.position.tokensOwed1)
  const gasEstimate = snapshot.gasEstimate === null
    ? null
  ...
  return parseArtifact({
    ...base(task.taskId, eligible || cooldownOnly ? "ANALYZED" : "PLAN_REJECTED", eligible ? "OUT_OF_RANGE" : cooldownOnly ? "COOLDOWN_ACTIVE" : "PLAN_CONSTRAINTS_FAILED", now, evidence),
    currentState,
...
function buildChecks(
  ...
  const tickPassed =
    ...
    bounds.lower >= task.allowedPool.minimumTick &&
    bounds.upper <= task.allowedPool.maximumTick &&
    width >= task.constraints.minimumRangeWidthTicks &&
    width <= task.constraints.maximumRangeWidthTicks
  return [
