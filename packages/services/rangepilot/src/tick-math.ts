...
export function sqrtRatioAtTick(tick: number): bigint {
  if (!Number.isInteger(tick) || tick < MIN_TICK || tick > MAX_TICK) {
    throw new RangeError("tick is outside the supported V3 domain")
  }
  ...
  for (const [mask, multiplier] of RATIOS) {
    if ((absoluteTick & mask) !== 0) ratio = (ratio * multiplier) >> 128n
  }
...
export function amountsForLiquidity(
  ...
  if (sqrtPriceX96 <= sqrtLower) {
    return { amount0: divCeil(liquidity * (sqrtUpper - sqrtLower) * Q96, sqrtUpper * sqrtLower), amount1: 0n }
  }
  if (sqrtPriceX96 < sqrtUpper) {
    return {
      amount0: divCeil(liquidity * (sqrtUpper - sqrtPriceX96) * Q96, sqrtUpper * sqrtPriceX96),
...
export function alignedFloor(tick: number, spacing: number): number {
  return Math.floor(tick / spacing) * spacing
}

export function alignedCeil(tick: number, spacing: number): number {
