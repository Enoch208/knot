...
const allowedPool = z
  ...
  .strict()
  .refine((value) => value.minimumTick < value.maximumTick, { message: "minimumTick must be below maximumTick" })

export const rangePilotTask = z
  .object({
  ...
  .refine(
    (value) =>
      value.constraints.minimumRangeWidthTicks <= value.constraints.targetRangeWidthTicks &&
      value.constraints.targetRangeWidthTicks <= value.constraints.maximumRangeWidthTicks,
    { message: "target range width must remain inside the accepted width" },
  )

const snapshotToken = tokenIdentity.extend({ balanceUnits: baseUnits }).strict()
const source = z.object({ uri: z.string().min(1), contentHash: digest, method: z.string().min(1) }).strict()

export const rangePilotSnapshot = z
  .object({
    ...
    gasEstimate: z
      .object({ gasUnits: baseUnits, gasPriceWei: baseUnits, estimatedAtUtc: utc, method: z.string().min(1) })
      .strict()
      .nullable(),
  })
  .strict()

export const rangePilotRequest = z
  .object({
    schemaVersion: z.literal("knot.rangepilot.request/1"),
    task: rangePilotTask,
    snapshot: rangePilotSnapshot,
...
const planCheck = z.object({ code: checkCode, passed: z.boolean(), observed: z.string(), limit: z.string() }).strict()
...
export const rangePilotArtifact = z
  .object({
    ...
    evidence: z
      .object({
        ...
        blockTimestampUtc: utc,
        capturedAtUtc: utc,
        gasEstimatedAtUtc: utc.nullable(),
      })
      .strict()
      .nullable(),
    currentState: z
...
export type RangePilotRequest = z.infer<typeof rangePilotRequest>
export type RangePilotArtifact = z.infer<typeof rangePilotArtifact>
export type RangePilotPlanCheck = z.infer<typeof planCheck>
