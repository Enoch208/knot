...
test("rejects oversized raw, encoded, and compressed task transports", () => {
  ...
  assert.equal(JSON.parse(analyzeGridQuantText(oversized, NOW)).reasonCode, "INVALID_JSON");
  assert.equal(JSON.parse(analyzeGridQuantText(encoded, NOW)).reasonCode, "INVALID_JSON");
  assert.equal(JSON.parse(analyzeGridQuantText(compressed, NOW)).reasonCode, "INVALID_JSON");
});
