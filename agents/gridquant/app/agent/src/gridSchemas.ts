...
const token = z
  ...
  .strict();
const pair = z
  .object({ chainId: z.literal(56), baseToken: token, quoteToken: token, pool: address })
  .strict();
const parameters = z
  .object({
    ...
    feeAssumptions: z
      .object({ buyFeeBps: bps, sellFeeBps: bps })
      .strict(),
    slippageBps: bps,
  ...
  .object({
    ...
    snapshot: z
      .object({
        ...
        sources: z
          .array(z.object({ uri: z.string().min(1), contentHash: digest, method: z.string().min(1) }).strict())
          .min(1),
        requester: address,
...
const noAction = z
  ...
  .strict();
const gridResult = z.discriminatedUnion("outcome", [plan, noAction]);
const evidence = z
  .object({
    snapshotId: z.string().min(1),
    chainId: z.literal(56),
...
export const gridQuantArtifact = z
  .object({
    ...
    reasonCode: z.enum([
      ...
      "GAS_EVIDENCE_MISMATCH",
      "GAS_EVIDENCE_UNAVAILABLE",
...
export type GridQuantArtifact = z.infer<typeof gridQuantArtifact>;
