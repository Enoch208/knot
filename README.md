...
This repository is a pre-production pilot. It contains a deployed control-plane API, one temporarily deployed BSC testnet seller, and four deterministic analysis services. Two funded testnet jobs reached submission: job `1181` produced a verified `NO_DEBT` artifact and reached terminal `COMPLETED` settlement, while disputed job `1180` produced an invalid-input artifact and returned its full escrow to the buyer through the expiry refund path. No comparative performance benchmark is claimed.
